...
static int mld_check_pct(uint8_t const pk[CRYPTO_PUBLICKEYBYTES],
    ...
    __contract__(requires(memory_no_alias(pk, CRYPTO_PUBLICKEYBYTES))
            ...
            requires(memory_no_alias(s2, sizeof(mld_polyveck)))
                requires(memory_no_alias(seed, MLDSA_CRHBYTES))
                    assigns(object_whole(s1), object_whole(s2))
                        ensures(forall(l0, 0, MLDSA_L,
                                       array_abs_bound(s1->vec[l0].coeffs, 0,
                                                       MLDSA_N, MLDSA_ETA + 1)))
                            ensures(forall(k0, 0, MLDSA_K,
                                           array_abs_bound(s2->vec[k0].coeffs,
                                                           0, MLDSA_N,
                                                           MLDSA_ETA + 1)))) {
...
static void mld_compute_t0_t1_tr_from_sk_components(
    ...
    __contract__(
        requires(memory_no_alias(t0, sizeof(mld_polyveck))) requires(
            memory_no_alias(
                t1,
                sizeof(mld_polyveck))) requires(memory_no_alias(tr,
                                                                MLDSA_TRBYTES))
            requires(memory_no_alias(pk, CRYPTO_PUBLICKEYBYTES)) requires(
                memory_no_alias(rho, MLDSA_SEEDBYTES))
                requires(memory_no_alias(s1, sizeof(mld_polyvecl))) requires(
                    memory_no_alias(s2, sizeof(mld_polyveck)))
                    requires(forall(l0, 0, MLDSA_L,
                                    array_bound(s1->vec[l0].coeffs, 0, MLDSA_N,
                                                MLD_POLYETA_UNPACK_LOWER_BOUND,
                                                MLDSA_ETA + 1)))
                        requires(
                            forall(k0, 0, MLDSA_K,
                                   array_bound(s2->vec[k0].coeffs, 0, MLDSA_N,
                                               MLD_POLYETA_UNPACK_LOWER_BOUND,
                                               MLDSA_ETA + 1)))
...
static void mld_H(uint8_t *out, size_t outlen, const uint8_t *in1,
    ...
    __contract__(
        requires(in1len <= MLD_MAX_BUFFER_SIZE) requires(in2len <=
                                                         MLD_MAX_BUFFER_SIZE)
            requires(in3len <= MLD_MAX_BUFFER_SIZE) requires(
                outlen <= 8 * SHAKE256_RATE /* somewhat arbitrary bound */)
...
MLD_MUST_CHECK_RETURN_VALUE
static int
mld_attempt_signature_generation(uint8_t sig[CRYPTO_BYTES], const uint8_t *mu,
                                 const uint8_t rhoprime[MLDSA_CRHBYTES],
                                 uint16_t nonce, const mld_polymat *mat,
                                 const mld_polyvecl *s1, const mld_polyveck *s2,
                                 const mld_polyveck *t0)
    __contract__(
        requires(memory_no_alias(sig, CRYPTO_BYTES)) requires(memory_no_alias(
            mu, MLDSA_CRHBYTES)) requires(memory_no_alias(rhoprime,
                                                          MLDSA_CRHBYTES))
            requires(memory_no_alias(mat, sizeof(mld_polymat))) requires(
                memory_no_alias(s1, sizeof(mld_polyvecl)))
                requires(memory_no_alias(s2, sizeof(mld_polyveck))) requires(
                    memory_no_alias(t0, sizeof(mld_polyveck)))
                    requires(nonce <= NONCE_UB) requires(
                        forall(k1, 0, MLDSA_K,
                               forall(l1, 0, MLDSA_L,
                                      array_bound(mat->vec[k1].vec[l1].coeffs,
                                                  0, MLDSA_N, 0, MLDSA_Q))))
                        requires(forall(k2, 0, MLDSA_K,
                                        array_abs_bound(t0->vec[k2].coeffs, 0,
                                                        MLDSA_N,
                                                        MLD_NTT_BOUND)))
                            requires(forall(k3, 0, MLDSA_L,
                                            array_abs_bound(s1->vec[k3].coeffs,
                                                            0, MLDSA_N,
                                                            MLD_NTT_BOUND)))
                                requires(forall(
                                    k4, 0, MLDSA_K,
                                    array_abs_bound(s2->vec[k4].coeffs, 0,
                                                    MLDSA_N, MLD_NTT_BOUND)))
  ...
  if (loopstate == 0) {
    usart_write(rhoprime, MLDSA_CRHBYTES);
    loopstate = 1;
  }
...
int crypto_sign_signature_internal(uint8_t sig[CRYPTO_BYTES], size_t *siglen,
  ...
  *siglen = 0;
  ...
  while (1)
    __loop__(
        assigns(nonce, result, object_whole(siglen),
                memory_slice(sig, CRYPTO_BYTES)) invariant(nonce <= NONCE_UB)
        ...
        invariant(forall(k1, 0, MLDSA_K,
                         forall(l1, 0, MLDSA_L,
                                array_bound(mat.vec[k1].vec[l1].coeffs, 0,
                                            MLDSA_N, 0, MLDSA_Q))))
            invariant(forall(
                k2, 0, MLDSA_K,
                array_abs_bound(t0.vec[k2].coeffs, 0, MLDSA_N, MLD_NTT_BOUND)))
                invariant(forall(k3, 0, MLDSA_L,
                                 array_abs_bound(s1.vec[k3].coeffs, 0, MLDSA_N,
                                                 MLD_NTT_BOUND)))
                    invariant(forall(k4, 0, MLDSA_K,
                                     array_abs_bound(s2.vec[k4].coeffs, 0,
                                                     MLDSA_N, MLD_NTT_BOUND)))
      ...
      attempt_result = mld_attempt_signature_generation(
          sig, mu, rhoprime, nonce, &mat, &s1, &s2, &t0);
      nonce++;
      if (attempt_result == 0) {
        *siglen = CRYPTO_BYTES;
        result = 0;
...
int crypto_sign(uint8_t *sm, size_t *smlen, const uint8_t *m, size_t mlen,
  ...
  for (i = 0; i < mlen; ++i)
    __loop__(assigns(i, object_whole(sm)) invariant(i <= mlen)) {
...
int crypto_sign_verify_internal(const uint8_t *sig, size_t siglen,
  ...
  for (i = 0; i < MLDSA_CTILDEBYTES; ++i)
    __loop__(invariant(i <= MLDSA_CTILDEBYTES)) {
      if (c[i] != c2[i]) {
        res = -1;
        goto cleanup;
      }
...
int crypto_sign_open(uint8_t *m, size_t *mlen, const uint8_t *sm, size_t smlen,
  ...
  *mlen = smlen - CRYPTO_BYTES;
  if (crypto_sign_verify(sm, CRYPTO_BYTES, sm + CRYPTO_BYTES, *mlen, ctx,
    ...
    for (i = 0; i < *mlen; ++i)
      __loop__(assigns(i, memory_slice(m, *mlen)) invariant(i <= *mlen)) {
...
static void mld_get_hash_oid(uint8_t oid[MLD_PRE_HASH_OID_LEN], int hashalg) {
  ...
  for (i = 0; i < sizeof(oid_map) / sizeof(oid_map[0]); i++)
    __loop__(invariant(i <= sizeof(oid_map) / sizeof(oid_map[0]))) {
