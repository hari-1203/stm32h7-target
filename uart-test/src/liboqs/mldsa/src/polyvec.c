 ...
 */
static void mld_polymat_permute_bitrev_to_custom(mld_polymat *mat) __contract__(
    ...
    requires(memory_no_alias(mat, sizeof(mld_polymat)))
        requires(forall(k1, 0, MLDSA_K,
                        forall(l1, 0, MLDSA_L,
                               array_bound(mat->vec[k1].vec[l1].coeffs, 0,
                                           MLDSA_N, 0, MLDSA_Q))))
            assigns(memory_slice(mat, sizeof(mld_polymat)))
                ensures(forall(k1, 0, MLDSA_K,
                               forall(l1, 0, MLDSA_L,
                                      array_bound(mat->vec[k1].vec[l1].coeffs,
                                                  0, MLDSA_N, 0, MLDSA_Q))))) {
...
void mld_polyvec_matrix_expand(mld_polymat *mat,
  ...
  for (j = 0; j < 4; j++)
    __loop__(assigns(j, object_whole(seed_ext)) invariant(j <= 4)) {
  ...
  for (i = 0; i < (MLDSA_K * MLDSA_L / 4) * 4; i += 4)
    __loop__(assigns(i, j, object_whole(seed_ext),
                     memory_slice(mat, sizeof(mld_polymat)))
                 invariant(i <= (MLDSA_K * MLDSA_L / 4) * 4 && i % 4 == 0)
             /* vectors 0 .. i / MLDSA_L are completely sampled */
             invariant(forall(k1, 0, i / MLDSA_L,
                              forall(l1, 0, MLDSA_L,
                                     array_bound(mat->vec[k1].vec[l1].coeffs, 0,
                                                 MLDSA_N, 0, MLDSA_Q))))
             /* last vector is sampled up to i % MLDSA_L */
             invariant(forall(k2, i / MLDSA_L, i / MLDSA_L + 1,
                              forall(l2, 0, i % MLDSA_L,
                                     array_bound(mat->vec[k2].vec[l2].coeffs, 0,
                                                 MLDSA_N, 0, MLDSA_Q))))) {
      for (j = 0; j < 4; j++)
        __loop__(assigns(j, object_whole(seed_ext)) invariant(j <= 4)) {
  ...
  while (i < MLDSA_K * MLDSA_L)
    __loop__(assigns(i, object_whole(seed_ext),
                     memory_slice(mat, sizeof(mld_polymat)))
...
void mld_polyvec_matrix_pointwise_montgomery(mld_polyveck *t,
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(assigns(i, memory_slice(t, sizeof(mld_polyveck))) invariant(
        i <= MLDSA_K) invariant(forall(k0, 0, i,
                                       array_abs_bound(t->vec[k0].coeffs, 0,
                                                       MLDSA_N, MLDSA_Q)))) {
...
void mld_polyvecl_reduce(mld_polyvecl *v) {
  ...
  for (i = 0; i < MLDSA_L; ++i)
    __loop__(assigns(i, memory_slice(v, sizeof(mld_polyvecl)))
                 invariant(i <= MLDSA_L) invariant(
                     forall(k0, i, MLDSA_L,
                            forall(k1, 0, MLDSA_N,
                                   v->vec[k0].coeffs[k1] ==
                                       loop_entry(*v).vec[k0].coeffs[k1])))
                     invariant(forall(k2, 0, i,
                                      array_bound(v->vec[k2].coeffs, 0, MLDSA_N,
                                                  -REDUCE32_RANGE_MAX,
                                                  REDUCE32_RANGE_MAX)))) {
...
void mld_polyvecl_add(mld_polyvecl *u, const mld_polyvecl *v) {
  ...
  for (i = 0; i < MLDSA_L; ++i)
    __loop__(assigns(i, memory_slice(u, sizeof(mld_polyvecl)))
                 invariant(i <= MLDSA_L) invariant(
                     forall(k0, i, MLDSA_L,
                            forall(k1, 0, MLDSA_N,
                                   u->vec[k0].coeffs[k1] ==
                                       loop_entry(*u).vec[k0].coeffs[k1])))
                     invariant(
                         forall(k6, 0, i,
                                array_bound(u->vec[k6].coeffs, 0, MLDSA_N,
...
void mld_polyvecl_ntt(mld_polyvecl *v) {
  ...
  for (i = 0; i < MLDSA_L; ++i)
    __loop__(
        assigns(i,
                memory_slice(v, sizeof(mld_polyvecl))) invariant(i <= MLDSA_L)
            invariant(forall(k0, i, MLDSA_L,
                             forall(k1, 0, MLDSA_N,
                                    v->vec[k0].coeffs[k1] ==
                                        loop_entry(*v).vec[k0].coeffs[k1])))
                invariant(forall(k1, 0, i,
                                 array_abs_bound(v->vec[k1].coeffs, 0, MLDSA_N,
                                                 MLD_NTT_BOUND)))) {
...
void mld_polyvecl_pointwise_poly_montgomery(mld_polyvecl *r, const mld_poly *a,
  ...
  for (i = 0; i < MLDSA_L; ++i)
    __loop__(assigns(i, memory_slice(r, sizeof(mld_polyvecl))) invariant(
        i <= MLDSA_L) invariant(forall(k2, 0, i,
                                       array_abs_bound(r->vec[k2].coeffs, 0,
                                                       MLDSA_N, MLDSA_Q)))) {
...
MLD_STATIC_TESTABLE void
mld_polyvecl_pointwise_acc_montgomery_c(mld_poly *w, const mld_polyvecl *u,
                                        const mld_polyvecl *v)
    __contract__(
            ...
            requires(memory_no_alias(u, sizeof(mld_polyvecl)))
                requires(memory_no_alias(v, sizeof(mld_polyvecl)))
                    requires(forall(l0, 0, MLDSA_L,
                                    array_bound(u->vec[l0].coeffs, 0, MLDSA_N,
                                                0, MLDSA_Q)))
                        requires(forall(l1, 0, MLDSA_L,
                                        array_abs_bound(v->vec[l1].coeffs, 0,
                                                        MLDSA_N,
                                                        MLD_NTT_BOUND)))
                            assigns(memory_slice(w, sizeof(mld_poly)))
                                ensures(array_abs_bound(w->coeffs, 0, MLDSA_N,
                                                        MLDSA_Q))) {
  unsigned int i, j;
  mld_assert_bound_2d(u->vec, MLDSA_L, MLDSA_N, 0, MLDSA_Q);
  mld_assert_abs_bound_2d(v->vec, MLDSA_L, MLDSA_N, MLD_NTT_BOUND);
  for (i = 0; i < MLDSA_N; i++)
    __loop__(assigns(i, j, memory_slice(w, sizeof(mld_poly))) invariant(
        i <= MLDSA_N) invariant(array_abs_bound(w->coeffs, 0, i, MLDSA_Q))) {
      int64_t t = 0;
      int32_t r;
      for (j = 0; j < MLDSA_L; j++)
        __loop__(assigns(j, t) invariant(j <= MLDSA_L) invariant(
            t >= -(int64_t)j * (MLDSA_Q - 1) * (MLD_NTT_BOUND - 1))
                     invariant(t <= (int64_t)j * (MLDSA_Q - 1) *
                                        (MLD_NTT_BOUND - 1))) {
...
uint32_t mld_polyvecl_chknorm(const mld_polyvecl *v, int32_t bound) {
  ...
  for (i = 0; i < MLDSA_L; ++i)
    __loop__(
        invariant(i <= MLDSA_L) invariant(t == 0 || t == 0xFFFFFFFF)
            invariant((t == 0) == forall(k1, 0, i,
                                         array_abs_bound(v->vec[k1].coeffs, 0,
                                                         MLDSA_N, bound)))) {
...
void mld_polyveck_reduce(mld_polyveck *v) {
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(assigns(i, memory_slice(v, sizeof(mld_polyveck)))
                 invariant(i <= MLDSA_K) invariant(
                     forall(k0, i, MLDSA_K,
                            forall(k1, 0, MLDSA_N,
                                   v->vec[k0].coeffs[k1] ==
                                       loop_entry(*v).vec[k0].coeffs[k1])))
                     ...
                     forall(k0, i, MLDSA_K,
                            forall(k1, 0, MLDSA_N,
                                   v->vec[k0].coeffs[k1] ==
                                       loop_entry(*v).vec[k0].coeffs[k1])))
                     invariant(forall(k1, 0, i,
                                      array_bound(v->vec[k1].coeffs, 0, MLDSA_N,
                                                  0, MLDSA_Q)))) {
...
void mld_polyveck_add(mld_polyveck *u, const mld_polyveck *v) {
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(assigns(i, memory_slice(u, sizeof(mld_polyveck)))
                 invariant(i <= MLDSA_K) invariant(
                     forall(k0, i, MLDSA_K,
                            forall(k1, 0, MLDSA_N,
                                   u->vec[k0].coeffs[k1] ==
...
void mld_polyveck_sub(mld_polyveck *u, const mld_polyveck *v) {
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(assigns(i, memory_slice(u, sizeof(mld_polyveck)))
                 invariant(i <= MLDSA_K) invariant(
                     forall(k0, 0, i,
                            array_bound(u->vec[k0].coeffs, 0, MLDSA_N,
                                        INT32_MIN, REDUCE32_DOMAIN_MAX)))
                     invariant(forall(
                         k1, i, MLDSA_K,
                         forall(n1, 0, MLDSA_N,
                                u->vec[k1].coeffs[n1] ==
                                    loop_entry(*u).vec[k1].coeffs[n1])))) {
...
void mld_polyveck_shiftl(mld_polyveck *v) {
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(
        assigns(i, memory_slice(v, sizeof(mld_polyveck))) invariant(
            i <= MLDSA_K) invariant(forall(k1, 0, i,
                                           array_bound(v->vec[k1].coeffs, 0,
                                                       MLDSA_N, 0, MLDSA_Q)))
            invariant(forall(k1, i, MLDSA_K,
                             forall(n1, 0, MLDSA_N,
                                    v->vec[k1].coeffs[n1] ==
                                        loop_entry(*v).vec[k1].coeffs[n1])))) {
...
void mld_polyveck_ntt(mld_polyveck *v) {
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(
        assigns(i,
                memory_slice(v, sizeof(mld_polyveck))) invariant(i <= MLDSA_K)
...
void mld_polyveck_pointwise_poly_montgomery(mld_polyveck *r, const mld_poly *a,
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(assigns(i, memory_slice(r, sizeof(mld_polyveck))) invariant(
        i <= MLDSA_K) invariant(forall(k2, 0, i,
                                       array_abs_bound(r->vec[k2].coeffs, 0,
                                                       MLDSA_N, MLDSA_Q)))) {
      mld_poly_pointwise_montgomery(&r->vec[i], a, &v->vec[i]);
    }
...
uint32_t mld_polyveck_chknorm(const mld_polyveck *v, int32_t bound) {
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(
        invariant(i <= MLDSA_K) invariant(t == 0 || t == 0xFFFFFFFF)
            invariant((t == 0) == forall(k1, 0, i,
                                         array_abs_bound(v->vec[k1].coeffs, 0,
                                                         MLDSA_N, bound)))) {
      /* Reference: Leaks which polynomial violates the bound via a conditional.
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(
        assigns(i, memory_slice(v0, sizeof(mld_polyveck)),
                memory_slice(v1, sizeof(mld_polyveck))) invariant(i <= MLDSA_K)
            invariant(forall(k1, 0, i,
                             array_bound(v0->vec[k1].coeffs, 0, MLDSA_N,
                                         -(MLD_2_POW_D / 2) + 1,
                                         (MLD_2_POW_D / 2) + 1)))
                invariant(
                    forall(k2, 0, i,
                           array_bound(v1->vec[k2].coeffs, 0, MLDSA_N, 0,
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(
        assigns(i, memory_slice(v0, sizeof(mld_polyveck)),
                memory_slice(v1, sizeof(mld_polyveck))) invariant(i <= MLDSA_K)
            invariant(forall(k1, 0, i,
                             array_bound(v1->vec[k1].coeffs, 0, MLDSA_N, 0,
                                         (MLDSA_Q - 1) / (2 * MLDSA_GAMMA2))))
                invariant(forall(k2, 0, i,
                                 array_abs_bound(v0->vec[k2].coeffs, 0, MLDSA_N,
                                                 MLDSA_GAMMA2 + 1)))) {
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(
        assigns(i, s, memory_slice(h, sizeof(mld_polyveck)))
            invariant(i <= MLDSA_K) invariant(s <= i * MLDSA_N) invariant(
                forall(k1, 0, i,
...
void mld_polyveck_use_hint(mld_polyveck *w, const mld_polyveck *u,
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(assigns(i, memory_slice(w, sizeof(mld_polyveck)))
                 invariant(i <= MLDSA_K) invariant(
                     forall(k2, 0, i,
...
void mld_polyveck_pack_w1(uint8_t r[MLDSA_K * MLDSA_POLYW1_PACKEDBYTES],
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(assigns(i, memory_slice(r, MLDSA_K * MLDSA_POLYW1_PACKEDBYTES))
                 invariant(i <= MLDSA_K)) {
...
void mld_polyveck_pack_eta(uint8_t r[MLDSA_K * MLDSA_POLYETA_PACKEDBYTES],
  ...
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(assigns(i, memory_slice(r, MLDSA_K * MLDSA_POLYETA_PACKEDBYTES))
                 invariant(i <= MLDSA_K)) {
...
void mld_polyvecl_pack_eta(uint8_t r[MLDSA_L * MLDSA_POLYETA_PACKEDBYTES],
  ...
  for (i = 0; i < MLDSA_L; ++i)
    __loop__(assigns(i, memory_slice(r, MLDSA_L * MLDSA_POLYETA_PACKEDBYTES))
                 invariant(i <= MLDSA_L)) {
      mld_polyeta_pack(&r[i * MLDSA_POLYETA_PACKEDBYTES], &p->vec[i]);
    }
}
...
void mld_polyvecl_pack_z(uint8_t r[MLDSA_L * MLDSA_POLYZ_PACKEDBYTES],
  ...
  mld_assert_bound_2d(p->vec, MLDSA_L, MLDSA_N, -(MLDSA_GAMMA1 - 1),
                      MLDSA_GAMMA1 + 1);
  for (i = 0; i < MLDSA_L; ++i)
    __loop__(assigns(i, memory_slice(r, MLDSA_L * MLDSA_POLYZ_PACKEDBYTES))
                 invariant(i <= MLDSA_L)) {
...
void mld_polyveck_pack_t0(uint8_t r[MLDSA_K * MLDSA_POLYT0_PACKEDBYTES],
  ...
  mld_assert_bound_2d(p->vec, MLDSA_K, MLDSA_N, -(1 << (MLDSA_D - 1)) + 1,
                      (1 << (MLDSA_D - 1)) + 1);
  for (i = 0; i < MLDSA_K; ++i)
    __loop__(assigns(i, memory_slice(r, MLDSA_K * MLDSA_POLYT0_PACKEDBYTES))
                 invariant(i <= MLDSA_K)) {
...
MLD_INTERNAL_API
void mld_polyveck_unpack_t0(
    mld_polyveck *p, const uint8_t r[MLDSA_K * MLDSA_POLYT0_PACKEDBYTES]) {
  unsigned int i;
  for (i = 0; i < MLDSA_K; ++i) {
