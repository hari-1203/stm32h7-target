...
static void mlk_polyvec_permute_bitrev_to_custom(mlk_polyvec *v) __contract__(
        ...
        requires(forall(x, 0, MLKEM_K,
            ...
            assigns(memory_slice(v, sizeof(mlk_polyvec)))
                ensures(forall(x, 0, MLKEM_K,
                               array_bound(v->vec[x].coeffs, 0, MLKEM_N, 0,
                                           MLKEM_Q)))) {
#if defined(MLK_USE_NATIVE_NTT_CUSTOM_ORDER)
  unsigned i;
  for (i = 0; i < MLKEM_K; i++)
    __loop__(assigns(i, memory_slice(v, sizeof(mlk_polyvec)))
                 invariant(i <= MLKEM_K) invariant(forall(
                     x, 0, MLKEM_K,
                     array_bound(v->vec[x].coeffs, 0, MLKEM_N, 0, MLKEM_Q)))) {
...
static void mlk_polymat_permute_bitrev_to_custom(mlk_polymat *a) __contract__(
    /* We don't specify that this should be a permutation, but only
     * that it does not change the bound established at the end of
     * mlk_gen_matrix. */
    requires(memory_no_alias(a, sizeof(mlk_polymat)))
        requires(forall(x, 0, MLKEM_K,
                        forall(y, 0, MLKEM_K,
                               array_bound(a->vec[x].vec[y].coeffs, 0, MLKEM_N,
                                           0, MLKEM_Q))))
            assigns(object_whole(a))
                ensures(forall(x, 0, MLKEM_K,
                               forall(y, 0, MLKEM_K,
                                      array_bound(a->vec[x].vec[y].coeffs, 0,
                                                  MLKEM_N, 0, MLKEM_Q))))) {
  unsigned i;
  for (i = 0; i < MLKEM_K; i++)
    __loop__(assigns(i, object_whole(a)) invariant(i <= MLKEM_K)
                ...
                requires(memory_no_alias(v, sizeof(mlk_polyvec)))
                    requires(memory_no_alias(vc, sizeof(mlk_polyvec_mulcache)))
                        requires(
                            forall(k0, 0, MLKEM_K,
                                   forall(k1, 0, MLKEM_K,
                                          array_bound(a->vec[k0].vec[k1].coeffs,
                                                      0, MLKEM_N, 0,
                                                      MLKEM_UINT12_LIMIT))))
                            assigns(object_whole(out))) {
  unsigned i;
  for (i = 0; i < MLKEM_K; i++)
    __loop__(assigns(i, object_whole(out)) invariant(i <= MLKEM_K)) {
