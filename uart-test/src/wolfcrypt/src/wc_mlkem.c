...
MlKemKey *wc_MlKemKey_New(int type, void *heap, int devId) {
  int ret;
  MlKemKey *key =
      (MlKemKey *)XMALLOC(sizeof(MlKemKey), heap, DYNAMIC_TYPE_TMP_BUFFER);
  if (key != NULL) {
...
#ifndef WOLFSSL_MLKEM_CACHE_A
    /* e (v) | a (m) */
    e = (sword16 *)XMALLOC((k + 1) * k * MLKEM_N * sizeof(sword16), key->heap,
                           DYNAMIC_TYPE_TMP_BUFFER);
#else
    /* e (v) */
    e = (sword16 *)XMALLOC(k * MLKEM_N * sizeof(sword16), key->heap,
                           DYNAMIC_TYPE_TMP_BUFFER);
#endif
#else
    /* e (v) */
 ...
 * @return  NOT_COMPILED_IN when key type is not supported.
 */
static int mlkemkey_encapsulate(MlKemKey *key, const byte *m, byte *r,
                                byte *c) {
  int ret = 0;
  sword16 *a = NULL;
  ...
  if ((ret == 0) && ((key->flags & MLKEM_FLAG_A_SET) != 0)) {
    ...
    for (i = 0; i < k; i++) {
      unsigned int j;
      for (j = 0; j < k; j++) {
        XMEMCPY(&a[(i * k + j) * MLKEM_N], &key->a[(j * k + i) * MLKEM_N],
                MLKEM_N * 2);
      }
    }
  } else
...
int wc_MlKemKey_EncapsulateWithRandom(MlKemKey *key, unsigned char *c,
                                      unsigned char *k, const unsigned char *m,
                                      int len) {
#ifdef WOLFSSL_MLKEM_KYBER
...
int wc_MlKemKey_EncodePublicKey(MlKemKey *key, unsigned char *out, word32 len) {
  ...
  if ((ret == 0) && ((key->flags & MLKEM_FLAG_PUB_SET) != MLKEM_FLAG_PUB_SET)) {
    ret = BAD_FUNC_ARG;
  }
