...
static int mlkem_get_noise_i(MLKEM_PRF_T *prf, int k, sword16 *vec2, byte *seed,
                             int i, int make);
...
#define MLKEM_MONT_RED(a)                                                      \
  (sword16)(((a) - (sword32)(((sword16)((sword16)(a) * (sword16)MLKEM_QINV)) * \
                             (sword32)MLKEM_Q)) >>                             \
            16)
...
#define MLKEM_BARRETT_RED(a)                                                   \
  (sword16)(                                                                   \
      (sword16)(a) -                                                           \
      (sword16)((sword16)(((sword32)((sword32)MLKEM_V * (sword16)(a))) >>      \
                          26) *                                                \
                (word16)MLKEM_Q))
...
void mlkem_encapsulate(const sword16 *t, sword16 *u, sword16 *v,
                       const sword16 *a, sword16 *y, const sword16 *e1,
                       const sword16 *e2, const sword16 *m, int k) {
  int i;
 ...
 */
int mlkem_keygen_seeds(sword16 *s, sword16 *t, MLKEM_PRF_T *prf, sword16 *tv,
                       int k, byte *rho, byte *sigma) {
  int i;
  int ret = 0;
  sword16 *ai = tv;
  sword16 *e = tv;
...
int mlkem_encapsulate_seeds(const sword16 *pub, MLKEM_PRF_T *prf, sword16 *u,
                            sword16 *tp, sword16 *y, int k, const byte *msg,
                            byte *seed, byte *coins) {
  int ret = 0;
  int i;
  sword16 *a = tp;
  sword16 *e1 = tp;
...
static int mlkem_gen_matrix_k2_avx2(sword16 *a, byte *seed, int transposed) {
...
#ifdef WOLFSSL_SMALL_STACK
  rand =
      (byte *)XMALLOC(4 * GEN_MATRIX_SIZE + 2, NULL, DYNAMIC_TYPE_TMP_BUFFER);
  state =
      (word64 *)XMALLOC(sizeof(word64) * 25 * 4, NULL, DYNAMIC_TYPE_TMP_BUFFER);
  ...
  mlkem_redistribute_21_rand_avx2(
      state, rand + 0 * GEN_MATRIX_SIZE, rand + 1 * GEN_MATRIX_SIZE,
  ...
  for (i = SHA3_128_BYTES; i < GEN_MATRIX_SIZE; i += SHA3_128_BYTES) {
    ...
    mlkem_redistribute_21_rand_avx2(
        state, rand + i + 0 * GEN_MATRIX_SIZE, rand + i + 1 * GEN_MATRIX_SIZE,
        rand + i + 2 * GEN_MATRIX_SIZE, rand + i + 3 * GEN_MATRIX_SIZE);
  }

  /* Sample random bytes to create a polynomial. */
  p = rand;
  ctr0 = mlkem_rej_uniform_n_avx2(a + 0 * MLKEM_N, MLKEM_N, p, GEN_MATRIX_SIZE);
  p += GEN_MATRIX_SIZE;
  ctr1 = mlkem_rej_uniform_n_avx2(a + 1 * MLKEM_N, MLKEM_N, p, GEN_MATRIX_SIZE);
  p += GEN_MATRIX_SIZE;
  ctr2 = mlkem_rej_uniform_n_avx2(a + 2 * MLKEM_N, MLKEM_N, p, GEN_MATRIX_SIZE);
  p += GEN_MATRIX_SIZE;
  ctr3 = mlkem_rej_uniform_n_avx2(a + 3 * MLKEM_N, MLKEM_N, p, GEN_MATRIX_SIZE);
  ...
  while ((ctr0 < MLKEM_N) || (ctr1 < MLKEM_N) || (ctr2 < MLKEM_N) ||
    ...
    p = rand;
    ctr0 += mlkem_rej_uniform_avx2(a + 0 * MLKEM_N + ctr0, MLKEM_N - ctr0, p,
                                   XOF_BLOCK_SIZE);
    p += GEN_MATRIX_SIZE;
    ctr1 += mlkem_rej_uniform_avx2(a + 1 * MLKEM_N + ctr1, MLKEM_N - ctr1, p,
                                   XOF_BLOCK_SIZE);
    p += GEN_MATRIX_SIZE;
    ctr2 += mlkem_rej_uniform_avx2(a + 2 * MLKEM_N + ctr2, MLKEM_N - ctr2, p,
                                   XOF_BLOCK_SIZE);
    p += GEN_MATRIX_SIZE;
    ctr3 += mlkem_rej_uniform_avx2(a + 3 * MLKEM_N + ctr3, MLKEM_N - ctr3, p,
                                   XOF_BLOCK_SIZE);
  }
...
static int mlkem_gen_matrix_k3_avx2(sword16 *a, byte *seed, int transposed) {
  ...
  for (k = 0; k < 2; k++) {
    ...
    p = rand;
    ctr0 =
        mlkem_rej_uniform_n_avx2(a + 0 * MLKEM_N, MLKEM_N, p, GEN_MATRIX_SIZE);
    p += GEN_MATRIX_SIZE;
    ctr1 =
        mlkem_rej_uniform_n_avx2(a + 1 * MLKEM_N, MLKEM_N, p, GEN_MATRIX_SIZE);
    p += GEN_MATRIX_SIZE;
    ctr2 =
        mlkem_rej_uniform_n_avx2(a + 2 * MLKEM_N, MLKEM_N, p, GEN_MATRIX_SIZE);
    p += GEN_MATRIX_SIZE;
    ctr3 =
        mlkem_rej_uniform_n_avx2(a + 3 * MLKEM_N, MLKEM_N, p, GEN_MATRIX_SIZE);
    /* Create more blocks if too many rejected. */
    while ((ctr0 < MLKEM_N) || (ctr1 < MLKEM_N) || (ctr2 < MLKEM_N) ||
           (ctr3 < MLKEM_N)) {
      sha3_blocksx4_avx2(state);
  ...
  while (ctr0 < MLKEM_N) {
    ...
    ctr0 +=
        mlkem_rej_uniform_avx2(a + ctr0, MLKEM_N - ctr0, rand, XOF_BLOCK_SIZE);
  }
...
static int mlkem_gen_matrix_k2_aarch64(sword16 *a, byte *seed, int transposed) {
  ...
  while ((ctr0 < MLKEM_N) || (ctr1 < MLKEM_N) || (ctr2 < MLKEM_N)) {
    ...
    p = (byte *)st;
    ctr0 += mlkem_rej_uniform_neon(a + 0 * MLKEM_N + ctr0, MLKEM_N - ctr0, p,
                                   XOF_BLOCK_SIZE);
    p += 25 * 8;
    ctr1 += mlkem_rej_uniform_neon(a + 1 * MLKEM_N + ctr1, MLKEM_N - ctr1, p,
                                   XOF_BLOCK_SIZE);
    p += 25 * 8;
    ctr2 += mlkem_rej_uniform_neon(a + 2 * MLKEM_N + ctr2, MLKEM_N - ctr2, p,
                                   XOF_BLOCK_SIZE);
  }
  ...
  while (ctr0 < MLKEM_N) {
    BlockSha3(state);
    ctr0 += mlkem_rej_uniform_neon(a + ctr0, MLKEM_N - ctr0, p, XOF_BLOCK_SIZE);
  }
 ...
 * @param  [in, out]  hash  SHA-3 object.
 */
void mlkem_hash_free(wc_Sha3 *hash) { wc_Sha3_256_Free(hash); }
 ...
 */
void mlkem_prf_init(wc_Shake *prf) { wc_InitShake256(prf, NULL, 0); }
 ...
 * @param  [in, out]  shake256  SHAKE-256 object.
 */
void mlkem_prf_free(wc_Shake *prf) { wc_Shake256_Free(prf); }
    ...
    for (j = 0; (ret == 0) && (j < k); j++) {
      ...
      if (ret == 0) {
        ...
        while (ctr < MLKEM_N) {
          ...
          /* Alg 7, Step 4-16. */
          ctr += mlkem_rej_uniform_c(a + j * MLKEM_N + ctr, MLKEM_N - ctr, rand,
                                     XOF_BLOCK_SIZE);
        }
      }
    }
...
#endif
    {
      ret = mlkem_gen_matrix_c(prf, a, WC_ML_KEM_1024_K, seed, transposed);
    }
#endif
  } else
#endif
...
static int mlkem_gen_matrix_i(MLKEM_PRF_T *prf, sword16 *a, int k, byte *seed,
  ...
  for (j = 0; (ret == 0) && (j < k); j++) {
    ...
    if (ret == 0) {
       ...
       * Alg 7, Step 4-16. */
      ctr =
          mlkem_rej_uniform_c(a + j * MLKEM_N, MLKEM_N, rand, GEN_MATRIX_SIZE);
      /* Create more blocks if too many rejected.
       * Alg 7, Step 4. */
      while (ctr < MLKEM_N) {
        /* Alg 7, Step 5. */
...
static void mlkem_cbd_eta3(sword16 *p, const byte *r) {
  ...
  for (i = 0; i < MLKEM_N; i += 4) {
    ...
    word32 t = (((word32)(r[0])) << 0) | (((word32)(r[1])) << 8) |
               (((word32)(r[2])) << 16);
    word32 d;
  ...
  for (i = 0; i < MLKEM_N; i += 8) {
    ...
    word64 t = (((word64)(r[0])) << 0) | (((word64)(r[1])) << 8) |
               (((word64)(r[2])) << 16) | (((word64)(r[3])) << 24) |
               (((word64)(r[4])) << 32) | (((word64)(r[5])) << 40);
    word64 d;
    /* Add second and third bits to first. */
...
static void mlkem_get_noise_x4_eta2_avx2(byte *rand, byte *seed, byte o) {
  ...
  mlkem_redistribute_16_rand_avx2(
      state, rand + 0 * ETA2_RAND_SIZE, rand + 1 * ETA2_RAND_SIZE,
      rand + 2 * ETA2_RAND_SIZE, rand + 3 * ETA2_RAND_SIZE);
}
#endif
...
static void mlkem_get_noise_x4_eta3_avx2(byte *rand, byte *seed) {
  ...
  mlkem_redistribute_17_rand_avx2(
      state, rand + 0 * PRF_RAND_SZ, rand + 1 * PRF_RAND_SZ,
      rand + 2 * PRF_RAND_SZ, rand + 3 * PRF_RAND_SZ);
  i = SHA3_256_BYTES;
  sha3_blocksx4_avx2(state);
  mlkem_redistribute_8_rand_avx2(
      state, rand + i + 0 * PRF_RAND_SZ, rand + i + 1 * PRF_RAND_SZ,
      rand + i + 2 * PRF_RAND_SZ, rand + i + 3 * PRF_RAND_SZ);
}
...
static int mlkem_get_noise_c(MLKEM_PRF_T *prf, int k, sword16 *vec1, int eta1,
                             sword16 *vec2, int eta2, sword16 *poly,
                             byte *seed) {
  int ret = 0;
  int i;
...
#endif
      usart_write(seed, WC_ML_KEM_SYM_SZ);
    if (poly == NULL) {
 ...
 * @return  Compressed value.
 */
#define TO_COMP_WORD_10(v, i, j, k) TO_COMP_WORD_VEC(v, i, j, k, 10, 0x3ff)
 ...
 * @return  Compressed value.
 */
#define TO_COMP_WORD_11(v, i, j, k) TO_COMP_WORD_VEC(v, i, j, k, 11, 0x7ff)
...
#define TO_COMP_WORD_10(v, i, j, k)                                            \
  ((((MLKEM_V54 << 10) * (v)[(i) * MLKEM_N + (j) + (k)]) + MLKEM_V54_HALF) >>  \
   54)
...
#define TO_COMP_WORD_11(v, i, j, k)                                            \
  ((((MLKEM_V53 << 11) * (v)[(i) * MLKEM_N + (j) + (k)]) + MLKEM_V53_HALF) >>  \
   53)
...
static void mlkem_vec_compress_10_c(byte *r, sword16 *v, unsigned int k) {
  ...
  for (i = 0; i < k; i++) {
    ...
    for (j = 0; j < MLKEM_N; j += 16) {
      ...
      r32[0] =
          t0 | ((word32)t1 << 10) | ((word32)t2 << 20) | ((word32)t3 << 30);
 ...
 * @param  [in]   k  Number of polynomials in vector.
 */
static void mlkem_vec_decompress_10_c(sword16 *v, const byte *b,
                                      unsigned int k) {
  unsigned int i;
  unsigned int j;
#ifdef WOLFSSL_MLKEM_SMALL
...
static void mlkem_vec_decompress_11_c(sword16 *v, const byte *b) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < MLKEM_N / 8; j++) {
      ...
      t[2] = (b[2] >> 6) | ((word16)b[3] << 2) | ((word16)b[4] << 10);
      t[3] = (b[4] >> 1) | ((word16)b[5] << 7);
      t[4] = (b[5] >> 4) | ((word16)b[6] << 4);
      t[5] = (b[6] >> 7) | ((word16)b[7] << 1) | ((word16)b[8] << 9);
      ...
      sword16 t2 = (b[2] >> 6) | ((word16)b[3] << 2) | ((word16)b[4] << 10);
      sword16 t3 = (b[4] >> 1) | ((word16)b[5] << 7);
      sword16 t4 = (b[5] >> 4) | ((word16)b[6] << 4);
      sword16 t5 = (b[6] >> 7) | ((word16)b[7] << 1) | ((word16)b[8] << 9);
 ...
 * @return  Compressed value.
 */
#define TO_COMP_WORD_4(p, i, j) TO_COMP_WORD(p, i, j, 4, 0xf)
 ...
 * @return  Compressed value.
 */
#define TO_COMP_WORD_5(p, i, j) TO_COMP_WORD(p, i, j, 5, 0x1f)
 ...
 * @return  Decompressed value.
 */
#define DECOMP_4(p, i, j, t) p[(i) + (j)] = ((word16)((t) * MLKEM_Q) + 8) >> 4
...
#define FROM_MSG_BIT(p, msg, i, j)                                             \
  ((p)[8 * (i) + (j)] =                                                        \
       (((sword16)0 - (sword16)(((msg)[i] >> (j)) & 1)) ^ mlkem_opt_blocker) & \
       MLKEM_Q_1_HALF)
...
#define TO_MSG_BIT(m, p, i, j)                                                 \
  (m)[i] |=                                                                    \
      ((word32)((MLKEM_V31_2 * (p)[8 * (i) + (j)]) + MLKEM_V31_HALF) >> 31)    \
      << (j)
 ...
 * @param  [in]   p    Polynomial.
 */
void mlkem_to_msg(byte *msg, sword16 *p) { mlkem_to_msg_neon(msg, p); }
#endif /* WOLFSSL_MLKEM_NO_DECAPSULATE */
#endif /* !(__aarch64__ && WOLFSSL_ARMASM) */
...
static void mlkem_from_bytes_c(sword16 *p, const byte *b, int k) {
  ...
  for (j = 0; j < k; j++) {
    for (i = 0; i < MLKEM_N / 2; i++) {
      p[2 * i + 0] =
          ((b[3 * i + 0] >> 0) | ((word16)b[3 * i + 1] << 8)) & 0xfff;
      p[2 * i + 1] =
          ((b[3 * i + 1] >> 4) | ((word16)b[3 * i + 2] << 4)) & 0xfff;
    }
    p += MLKEM_N;
